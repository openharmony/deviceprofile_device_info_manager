...
void IpcUtils::SecureClearString(std::string& str)
{
    if (str.empty()) {
        return;
    }
...
void IpcUtils::SecureClearVector(std::vector<uint8_t>& vec)
{
    if (vec.empty()) {
        return;
    }
...
bool IpcUtils::ValidateStringLength(const std::string& str)
{
    if (str.length() > MAX_STRING_LENGTH) {
        HILOGE("String length exceeds limit! length: %{public}zu, max: %{public}zu", str.length(), MAX_STRING_LENGTH);
        return false;
    }
    return true;
}
...
bool IpcUtils::Marshalling(MessageParcel& parcel, const std::vector<std::string>& strings)
{
    ...
    for (const auto& item : strings) {
        if (!ValidateStringLength(item)) {
            HILOGE("String item length exceeds limit!");
            return false;
        }
...
bool IpcUtils::Marshalling(MessageParcel& parcel, const std::map<std::string, std::string>& params)
{
    ...
    for (const auto& item : params) {
        if (!ValidateStringLength(item.first) || !ValidateStringLength(item.second)) {
            HILOGE("Map key or value length exceeds limit!");
            return false;
        }
...
bool IpcUtils::UnMarshalling(MessageParcel& parcel, std::map<std::string, std::string>& params)
{
    ...
    for (uint32_t i = 0; i < size; i++) {
        ...
        if (!ValidateStringLength(key) || !ValidateStringLength(value)) {
            HILOGE("Map key or value length exceeds limit!");
            SecureClearString(item);
