...
#include "content_sensor_manager_utils_Refactor.h"
...
namespace {
    ...
    constexpr int32_t INVALID_PROT_TYPE = -1;
}

void ContentSensorManagerUtils::SecureClearMemory(void* ptr, size_t size)
{
...
std::string ContentSensorManagerUtils::ObtainProductModel()
{
    ...
    free(const_cast<char*>(productModelTemp));
    return deviceModel_;
}
...
std::string ContentSensorManagerUtils::ObtainDeviceType()
{
    ...
    free(const_cast<char*>(deviceTypeTemp));
    return deviceType_;
}
...
std::string ContentSensorManagerUtils::ObtainSerial()
{
    ...
    serial_ = serialTemp;
    size_t serialLen = strlen(serialTemp);
    SecureClearMemory(const_cast<char*>(serialTemp), serialLen);
    free(const_cast<char*>(serialTemp));
    return serial_;
}
...
std::string ContentSensorManagerUtils::ObtainMarketName()
{
    ...
    free(const_cast<char*>(marketNameTemp));
    return marketName_;
}
...
std::string ContentSensorManagerUtils::ObtainOsFullName()
{
    ...
    free(const_cast<char*>(osFullNameTemp));
    return osFullName_;
}
...
std::string ContentSensorManagerUtils::ObtainDisplayVersion()
{
    ...
    free(const_cast<char*>(displayVersionTemp));
    return displayVersion_;
}
...
std::string ContentSensorManagerUtils::ObtainLocalUdid()
{
    ...
    SecureClearMemory(localUdidTemp, DEVICE_UUID_LENGTH);
    return localUdid_;
}
...
void ContentSensorManagerUtils::ObtainDeviceDataSyncMode()
{
    ...
    if (ret > 0 && strncmp(isE2EDeviceParam, SYNC_TYPE_E2E, strlen(SYNC_TYPE_E2E)) == 0) {
        ...
        SecureClearMemory(isE2EDeviceParam, SYS_SETTINGS_DATA_SYNC_PARAM_LEN + 1);
        return;
    }
    SecureClearMemory(isE2EDeviceParam, SYS_SETTINGS_DATA_SYNC_PARAM_LEN + 1);
...
{
    std::lock_guard<std::mutex> lock(csMutex_);
    if (protType_ != INVALID_PROT_TYPE && protType_ > 0) {
        return protType_;
    }
    if (IsWifiOnly()) {
...
bool ContentSensorManagerUtils::IsWifiOnly()
{
    ...
    if (fgets(buf, WIFI_ONLY_FLAG_VALUE_MAX_LEN, fp) == nullptr) {
        ...
        fclose(fp);
        SecureClearMemory(buf, WIFI_ONLY_FLAG_VALUE_MAX_LEN);
        return false;
    }
    bool result = (strcmp(buf, WIFI_ONLY_FLAG_VALUE) == 0);
    if (fclose(fp) != 0) {
