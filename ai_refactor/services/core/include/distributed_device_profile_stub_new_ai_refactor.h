...
private:
    ...
    // 安全增强：参数验证方法（原则3、5、14）
    bool ValidateDeviceId(const std::string& deviceId);
    bool ValidateAclProfile(const AccessControlProfile& aclProfile);
    bool ValidateSessionKeyParams(uint32_t userId, int32_t sessionKeyId);
    bool ValidateUserId(int32_t userId);
    bool ValidateServiceInfoParams(int32_t saId, const sptr<IRemoteObject>& callback);
