...
constexpr int32_t MIN_VALID_SA_ID = 1;
constexpr int32_t MAX_VALID_SA_ID = 1000;
constexpr int32_t MIN_SESSION_KEY_ID = 0;
}
...
void DistributedDeviceProfileStubNew::InitAclAndSubscribe()
{
    ...
    aclAndSubscribeFuncs_.insert(static_cast<uint32_t>(DpIpcInterfaceCode::SUBSCRIBE_SERVICE_INFO_CALLBACK));
    aclAndSubscribeFuncs_.insert(static_cast<uint32_t>(DpIpcInterfaceCode::PUT_SERVICE_INFO));
...
// 安全增强：设备ID验证（原则3）
bool DistributedDeviceProfileStubNew::ValidateDeviceId(const std::string& deviceId)
{
    if (deviceId.empty()) {
    ...
    if (deviceId.length() < MIN_DEVICE_ID_LENGTH || deviceId.length() > MAX_DEVICE_ID_LENGTH) {
        HILOGE("Device ID length invalid: %{public}zu", deviceId.length());
        return false;
    }
    return true;
}
...
// 安全增强：ACL配置验证（原则3、15）
bool DistributedDeviceProfileStubNew::ValidateAclProfile(const AccessControlProfile& aclProfile)
{
    if (aclProfile.GetAclId() <= 0) {
        HILOGE("Invalid ACL ID");
        return false;
    }
    if (!ValidateDeviceId(aclProfile.GetDeviceId())) {
        HILOGE("Invalid device ID in ACL profile");
        return false;
    }
    return true;
}
...
// 安全增强：会话密钥参数验证（原则4、5）
bool DistributedDeviceProfileStubNew::ValidateSessionKeyParams(uint32_t userId, int32_t sessionKeyId)
{
    if (userId == 0) {
        HILOGE("Invalid user ID: 0");
        return false;
    }
    if (sessionKeyId < MIN_SESSION_KEY_ID) {
        HILOGE("Invalid session key ID: %{public}d", sessionKeyId);
        return false;
    }
    return true;
}
...
// 安全增强：用户ID验证（原则18）
bool DistributedDeviceProfileStubNew::ValidateUserId(int32_t userId)
{
    if (userId <= 0) {
        HILOGE("Invalid user ID: %{public}d", userId);
        return false;
    }
    return true;
}
...
// 安全增强：服务信息参数验证（原则19）
bool DistributedDeviceProfileStubNew::ValidateServiceInfoParams(int32_t saId, const sptr<IRemoteObject>& callback)
{
    if (saId < MIN_VALID_SA_ID || saId > MAX_VALID_SA_ID) {
...
{
    switch (code) {
        case static_cast<uint32_t>(DpIpcInterfaceCode::SUBSCRIBE_SERVICE_INFO_CALLBACK):
            return SubscribeAllServiceInfoInner(data, reply);
        case static_cast<uint32_t>(DpIpcInterfaceCode::PUT_SERVICE_INFO):
            return PutServiceInfoInner(data, reply);
        case static_cast<uint32_t>(DpIpcInterfaceCode::DELETE_SERVICE_INFO):
            return DeleteServiceInfoInner(data, reply);
        case static_cast<uint32_t>(DpIpcInterfaceCode::GET_ALL_SERVICE_INFO_LIST):
            return GetAllServiceInfoListInner(data, reply);
        case static_cast<uint32_t>(DpIpcInterfaceCode::GET_SERVICE_INFO_BY_USER_INFO):
            return GetServiceInfosByUserInfoInner(data, reply);
        default:
...
int32_t DistributedDeviceProfileStubNew::PutAccessControlProfileInner(MessageParcel& data, MessageParcel& reply)
{
    ...
    // 安全增强：ACL配置验证（原则3、15）
    if (!ValidateAclProfile(accessControlProfile)) {
        HILOGE("ACL profile validation failed");
...
int32_t DistributedDeviceProfileStubNew::GetTrustDeviceProfileInner(MessageParcel& data, MessageParcel& reply)
{
    ...
    if (!ValidateDeviceId(deviceId)) {
        HILOGE("Device ID validation failed");
        return DP_INVALID_PARAM;
    }
...
int32_t DistributedDeviceProfileStubNew::PutSessionKeyInner(MessageParcel& data, MessageParcel& reply)
{
    ...
    // 安全增强：会话密钥参数验证（原则4、5）
    if (!ValidateSessionKeyParams(userId, 0)) {
...
int32_t DistributedDeviceProfileStubNew::GetSessionKeyInner(MessageParcel& data, MessageParcel& reply)
{
    ...
    // 安全增强：会话密钥参数验证（原则4、5）
    if (!ValidateSessionKeyParams(userId, sessionKeyId)) {
        ProfileUtils::SecureClearSessionKey(sessionKey);
        return DP_INVALID_PARAM;
    }
...
int32_t DistributedDeviceProfileStubNew::PutServiceProfileInner(MessageParcel& data, MessageParcel& reply)
{
    ...
    // 安全增强：设备ID验证（原则3）
    if (!ValidateDeviceId(serviceProfile.GetDeviceId())) {
        HILOGE("Device ID validation failed");
        return DP_INVALID_PARAM;
    }
...
int32_t DistributedDeviceProfileStubNew::PutServiceProfileBatchInner(MessageParcel& data, MessageParcel& reply)
{
    ...
    for (const auto& profile : serviceProfiles) {
        if (!ValidateDeviceId(profile.GetDeviceId())) {
            HILOGE("Device ID validation failed in batch");
            return DP_INVALID_PARAM;
        }
    }
...
int32_t DistributedDeviceProfileStubNew::PutCharacteristicProfileInner(MessageParcel& data, MessageParcel& reply)
{
    ...
    // 安全增强：设备ID验证（原则3）
    if (!ValidateDeviceId(charProfile.GetDeviceId())) {
        HILOGE("Device ID validation failed");
        return DP_INVALID_PARAM;
    }
...
int32_t DistributedDeviceProfileStubNew::DeleteServiceProfileInner(MessageParcel& data, MessageParcel& reply)
{
    ...
    READ_HELPER(data, Int32, userId);
    // 安全增强：用户ID验证（原则18）
    if (isMultiUser && !ValidateUserId(userId)) {
        HILOGE("User ID validation failed");
        return DP_INVALID_PARAM;
    }
...
int32_t DistributedDeviceProfileStubNew::PutAllTrustedDevicesInner(MessageParcel& data, MessageParcel& reply)
{
    ...
    for (const auto& deviceInfo : deviceInfos) {
        if (!ValidateDeviceId(deviceInfo.deviceId)) {
            HILOGE("Device ID validation failed in trusted devices");
            return DP_INVALID_PARAM;
        }
    }
...
int32_t DistributedDeviceProfileStubNew::PutServiceInfoInner(MessageParcel& data, MessageParcel& reply)
{
    ...
    if (!serviceInfo.UnMarshalling(data)) {
        HILOGE("serviceInfo read parcel fail!");
        return DP_READ_PARCEL_FAIL;
    }

    int32_t ret = PutServiceInfo(serviceInfo);
    if (!reply.WriteInt32(ret)) {
        HILOGE("Write reply failed");
        return DP_WRITE_PARCEL_FAIL;
    }
...
int32_t DistributedDeviceProfileStubNew::DeleteServiceInfoInner(MessageParcel& data, MessageParcel& reply)
{
    UserInfo userInfo;
    if (!userInfo.UnMarshalling(data)) {
        HILOGE("read parcel fail!");
        return DP_READ_PARCEL_FAIL;
    }
    // 安全增强：用户ID验证（原则18）
    ...
    int32_t ret = DeleteServiceInfo(userInfo);
    if (!reply.WriteInt32(ret)) {
        HILOGE("Write reply failed");
        return ERR_FLATTEN_OBJECT;
    }
...
int32_t DistributedDeviceProfileStubNew::GetAllServiceInfoListInner(MessageParcel& data, MessageParcel& reply)
{
    std::vector<ServiceInfo> serviceInfos;
    int32_t ret = GetAllServiceInfoList(serviceInfos);
    if (!reply.WriteInt32(ret)) {
        HILOGE("Write reply failed");
        return ERR_FLATTEN_OBJECT;
    }

    if (!IpcUtils::Marshalling(reply, serviceInfos)) {
        return DP_READ_PARCEL_FAIL;
    }
    return DP_SUCCESS;
}

int32_t DistributedDeviceProfileStubNew::GetServiceInfosByUserInfoInner(MessageParcel& data, MessageParcel& reply)
{
    UserInfo userInfo;
    std::vector<ServiceInfo> serviceInfos;
    if (!userInfo.UnMarshalling(data)) {
    ...
    int32_t ret = GetServiceInfosByUserInfo(userInfo, serviceInfos);
    if (!reply.WriteInt32(ret)) {
        HILOGE("userInfo Write reply failed");
        return ERR_FLATTEN_OBJECT;
    }
    if (!IpcUtils::Marshalling(reply, serviceInfos)) {
        HILOGE("serviceInfos write parcel fail!");
        return DP_READ_PARCEL_FAIL;
    }
    return DP_SUCCESS;
}

int32_t DistributedDeviceProfileStubNew::SubscribeAllServiceInfoInner(MessageParcel& data, MessageParcel& reply)
{
    int32_t saId = -1;
    READ_HELPER(data, Int32, saId);
    sptr<IRemoteObject> listen = data.ReadRemoteObject();
    // 安全增强：服务信息参数验证（原则19）
    if (!ValidateServiceInfoParams(saId, listen)) {
        HILOGE("Service info params validation failed");
        return ERR_FLATTEN_OBJECT;
    }

    int32_t ret = SubscribeAllServiceInfo(saId, listen);
    if (!reply.WriteInt32(ret)) {
        HILOGE("Write reply failed");
        return ERR_FLATTEN_OBJECT;
    }
