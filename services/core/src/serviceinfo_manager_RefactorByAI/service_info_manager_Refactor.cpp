...
namespace {
    ...
    constexpr size_t MAX_CUSTOM_DATA_LEN = 4096;
    constexpr size_t MAX_FIELD_LEN = 1024;
    ...
    void SecureClearStringMap(std::map<std::string, std::string>& map)
    {
        for (auto& pair : map) {
            SecureClearString(pair.first);
            SecureClearString(pair.second);
        }
        map.clear();
    }
    
    void SecureClearCharArray(char* arr, size_t len)
    {
        if (arr != nullptr && len > 0) {
            (void)memset_s(arr, len, 0, len);
        }
    }
}
...
int32_t ServiceInfoManager::PutToLocalStore(const std::map<std::string, std::string>& entries)
{
    std::lock_guard<std::mutex> lock(storeMutex_);
    if (serviceInfoKvAdapter_ == nullptr) {
...
int32_t ServiceInfoManager::PutToSyncStore(const std::map<std::string, std::string>& entries)
{
    std::lock_guard<std::mutex> lock(syncStoreMutex_);
    if (syncServiceInfoKvAdapter_ == nullptr) {
        HILOGE("deviceProfileStore is nullptr!");
...
int32_t ServiceInfoManager::PutServiceInfo(const ServiceInfo &serviceInfo)
{
    ...
    int32_t ret = PutToLocalStore(entries);
    if (ret != DP_SUCCESS) {
        SecureClearStringMap(entries);
        return ret;
    }
    ...
    SecureClearCharArray(localUdidTemp, DEVICE_UUID_LENGTH);

    if (serviceInfo.GetUdid() != localUdid) {
        HILOGI("Non-local data");
        SecureClearString(localUdid);
        SecureClearStringMap(entries);
        return DP_SUCCESS;
    }

    ret = PutToSyncStore(entries);
    SecureClearString(localUdid);
    SecureClearStringMap(entries);
    if (ret != DP_SUCCESS) {
        return ret;
...
int32_t ServiceInfoManager::GetMatchedKeysByUserInfo(const UserInfo& userInfo, std::vector<std::string>& matchedKeys)
{
...
int32_t ServiceInfoManager::DeleteServiceInfo(const UserInfo& userInfo)
{
    ...
    int32_t ret = GetMatchedKeysByUserInfo(userInfo, matchedKeys);
    if (ret != DP_SUCCESS) {
        return ret;
    }
    ...
    ret = DeleteSyncServiceInfo(matchedKeys);
    if (ret != DP_SUCCESS) {
        return ret;
    }
...
std::vector<DistributedKv::Entry> ServiceInfoManager::GetEntriesByKeys(const std::vector<std::string>& keys)
{
    ...
    }
    {
        ...
        for (const auto& key : keys) {
            ...
            HILOGD("Found value for key: %{public}s",
...
bool ServiceInfoManager::ValidateStringFields(const ParsedJSONFields& fields)
{
    ...
    if (strlen(fields.customDataItem->valuestring) > MAX_CUSTOM_DATA_LEN) {
        HILOGE("customData field exceeds maximum length");
        return false;
    }
    
    if (strlen(fields.serviceNameItem->valuestring) > MAX_FIELD_LEN ||
        strlen(fields.serviceDisplayNameItem->valuestring) > MAX_FIELD_LEN ||
        strlen(fields.serviceCodeItem->valuestring) > MAX_FIELD_LEN ||
        strlen(fields.extraDataItem->valuestring) > MAX_CUSTOM_DATA_LEN) {
        HILOGE("Field exceeds maximum length");
        return false;
    }
