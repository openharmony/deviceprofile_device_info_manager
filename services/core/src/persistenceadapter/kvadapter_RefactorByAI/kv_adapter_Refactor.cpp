...
namespace {
    ...
    // [原则12] 修正注释，200*1000微秒=200ms，非500ms
...
int32_t KVAdapter::Init()
{
    ...
    // [原则12] 确保isInited_状态一致性
    if (isInited_.load()) {
        HILOGI("KVAdapter already inited.");
        return DP_SUCCESS;
    }
    ...
    // [原则12] 重试失败后确保isInited_保持false
    HILOGE("Init KvStorePtr failed after all retries");
...
int32_t KVAdapter::UnInit()
{
    ...
    // [原则12] 确保只在已初始化状态下执行清理
    if (!isInited_.load()) {
        HILOGI("KVAdapter already uninited.");
        return DP_SUCCESS;
    }
...
int32_t KVAdapter::Put(const std::string& key, const std::string& value)
{
    ...
    DistributedKv::Status status;
    {
        ...
        if (kvStorePtr_->Get(kvKey, oldV) == DistributedKv::Status::SUCCESS && oldV.ToString() == value) {
            HILOGI("The key-value pair already exists. key=%{public}s",
