 ...
 */
#include "business_event_adapter_Refactor.h"
...
int32_t BusinessEventAdapter::Init()
{
    ...
    // [原则12] 重试失败后，校验kvStorePtr_状态，确保不进入不一致状态
    {
        std::lock_guard<std::mutex> lock(BusinessAdapterMutex_);
        if (kvStorePtr_ == nullptr) {
            HILOGE("kvStorePtr is nullptr after all retries!");
            return DP_KV_DB_PTR_NULL;
        }
    }
    // [原则12] 重试全部失败但kvStorePtr_非空（可能是残留状态），不设置isInited_
    HILOGE("GetKvStorePtr failed after all retries");
    return DP_KV_DB_PTR_NULL;
}
...
void BusinessEventAdapter::OnRemoteDied()
{
    ...
    // [原则12] BusinessEventAdapter为单例，生命周期与进程一致，lambda捕获this安全
...
int32_t BusinessEventAdapter::RegisterKvStoreDeathListener()
{
    ...
    // [原则12] 使用互斥锁保护监听器注册，与Unregister保持一致
    {
        std::lock_guard<std::mutex> lock(BusinessAdapterMutex_);
