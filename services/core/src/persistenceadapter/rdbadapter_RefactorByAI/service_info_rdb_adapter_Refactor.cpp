...
int32_t ServiceInfoRdbAdapter::UnInit()
{
    HILOGI("ServiceInfoRdbAdapter unInit");
    {
        ...
        if (store_ != nullptr) {
            int32_t closeRet = store_->Close();
            if (closeRet != E_OK) {
                HILOGW("RDBStore close failed ret:%{public}d", closeRet);
            }
            store_ = nullptr;
        }
    }
...
int32_t ServiceInfoRdbAdapter::Put(int64_t& outRowId, const std::string& table, const ValuesBucket& values)
{
    ...
    }
    {
        ...
        if (ret == E_SQLITE_CORRUPT) {
            ...
            if (ret == E_SQLITE_CORRUPT) {
                HILOGE("database still corrupt after restore ret:%{public}d", ret);
                return DP_RDB_DATABASE_RESTORE_FAIL;
            }
        }
        if (ret != E_OK) {
...
std::shared_ptr<ResultSet> ServiceInfoRdbAdapter::Get(const std::string& sql, const std::vector<ValueObject>& args)
{
    std::shared_ptr<ResultSet> resultSet = nullptr;
    {
        ...
        if (ret == E_SQLITE_CORRUPT) {
            ...
            resultSet = store_->QueryByStep(sql, args);
            if (resultSet == nullptr) {
                HILOGE("resultSet is null after restore");
                return nullptr;
            }
...
int32_t ServiceInfoRdbAdapter::GetRDBPtr()
{
    ...
    int32_t errCode = E_OK;
    {
        ...
        if (rebuiltType == NativeRdb::RebuiltType::REBUILT) {
            ...
            NativeRdb::RebuiltType verifyRebuiltType = NativeRdb::RebuiltType::NONE;
            int32_t verifyRet = store_->GetRebuilt(verifyRebuiltType);
            if (verifyRet != E_OK || verifyRebuiltType == NativeRdb::RebuiltType::REBUILT) {
                HILOGE("database still corrupt after restore verifyRet:%{public}d", verifyRet);
                return DP_RDB_DATABASE_RESTORE_FAIL;
            }
        }
    }
