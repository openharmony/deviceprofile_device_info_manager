...
#include "profile_data_manager_Refactor.h"
...
{
    // [原则12] 检查初始化状态，未初始化时无需执行清理操作
    if (!isInited_.load()) {
        HILOGI("ProfileDataManager not inited, skip UnInit");
        return DP_SUCCESS;
    }
    // [原则12] 反向顺序清理各DAO，与Init()顺序对称，保证资源释放顺序正确
...
// [原则3] 检测路径穿越模式：当字符串同时包含SEPARATOR和SLASHES时，判定为路径穿越尝试，返回true
// 在PutDeviceProfile中用于拒绝包含路径穿越模式的deviceId
...
int32_t ProfileDataManager::PutDeviceProfileBatch(std::vector<DeviceProfile>& deviceProfiles)
{
    ...
    for (auto deviceProfile : deviceProfiles) {
        ...
        // [原则12] 修复：原代码错误地检查ret而非putRet，导致第一个失败后的所有结果被忽略
