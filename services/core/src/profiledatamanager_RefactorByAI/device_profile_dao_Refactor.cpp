...
{
    // [原则3] 校验数据合法性，id必须有效（>0）才能按id更新
    if (newProfile.GetId() <= 0) {
        HILOGE("invalid id:%{public}d", newProfile.GetId());
        return DP_INVALID_PARAMS;
    }
    ValuesBucket values;
...
void DeviceProfileDao::GenerateSqlAndCondition(const std::vector<int32_t> &params,
    std::string &sql, std::vector<ValueObject> &condition)
{
    // [原则7] 限制IN条件元素数量，防止超大SQL导致资源耗尽或超出SQLite占位符上限
    if (params.size() > MAX_IN_QUERY_SIZE) {
        HILOGE("int32_t params size too large:%{public}zu, truncating to max", params.size());
    }
    uint32_t count = 0;
    for (auto param : params) {
        if (count >= MAX_IN_QUERY_SIZE) {
            break;
        }
        sql += "?,";
    ...
    std::string &sql, std::vector<ValueObject> &condition)
{
    // [原则7] 限制IN条件元素数量，防止超大SQL导致资源耗尽或超出SQLite占位符上限
    if (params.size() > MAX_IN_QUERY_SIZE) {
        HILOGE("string params size too large:%{public}zu, truncating to max", params.size());
    }
    uint32_t count = 0;
...
// [原则3] 校验DeviceProfile的合法性：deviceId不可为空或超长
bool DeviceProfileDao::IsValidDeviceProfile(const DeviceProfile& deviceProfile) const
{
    if (deviceProfile.GetDeviceId().empty() || deviceProfile.GetDeviceId().size() > MAX_STRING_LEN) {
