...
void KvDataChangeListener::OnChange(const DistributedKv::DataOrigin& origin, Keys&& keys)
{
    if (origin.store == STATIC_STORE_ID) {
        ...
        // [原则12] detached线程中lambda捕获this：KvDataChangeListener生命周期由KVAdapter管理，
        // KVAdapter在UnInit时等待同步完成，保证detach线程结束前对象不被销毁
...
void KvDataChangeListener::ProcessChangeOp(const std::vector<std::string>& keyList, ChangeOp op)
{
    ...
    // [原则12] GetDevUdid使用固定缓冲区DEVICE_UUID_LENGTH(65)，与UUID标准长度匹配，
    // 超长结果会被截断，不会溢出
...
// [原则12] cJSON资源清理：ProcessInsertOrUpdate中每个循环迭代的jsonObj在continue或正常结束时均调用cJSON_Delete
...
// [原则3] HandleSwitchUpdateChange中校验switchValue位范围，防止越界访问
