...
int32_t SubscribeProfileManager::NotifyDeviceAclInactiveByDelete(const TrustDeviceProfile& trustDeviceProfile)
{
    auto subscriberInfos = GetSubscribeInfos(SUBSCRIBE_TRUST_DEVICE_PROFILE);
    if (subscriberInfos.empty()) {
        return DP_SUCCESS;
    ...
    for (const auto& subscriberInfo : subscriberInfos) {
        ...
        if (subscriberInfo.GetProfileChangeTypes().count(ProfileChangeType::DEVICE_ACL_INACTIVE_BY_DELETE) != 0) {
            listenerProxy->OnDeviceAclInactiveByDelete(trustDeviceProfile);
        }
    }
    return DP_SUCCESS;
}

int32_t SubscribeProfileManager::NotifyDeviceAclInactiveByUpdate(const TrustDeviceProfile& trustDeviceProfile)
{
    auto subscriberInfos = GetSubscribeInfos(SUBSCRIBE_TRUST_DEVICE_PROFILE);
    if (subscriberInfos.empty()) {
        return DP_SUCCESS;
    ...
    for (const auto& subscriberInfo : subscriberInfos) {
        ...
        if (subscriberInfo.GetProfileChangeTypes().count(ProfileChangeType::DEVICE_ACL_INACTIVE_BY_UPDATE) != 0) {
            listenerProxy->OnDeviceAclInactiveByUpdate(trustDeviceProfile);
        }
    }
    return DP_SUCCESS;
}
...
int32_t SubscribeProfileManager::NotifyAccountAclAdd(const TrustDeviceProfile& trustDeviceProfile)
{
    auto subscriberInfos = GetSubscribeInfos(SUBSCRIBE_TRUST_DEVICE_PROFILE);
    if (subscriberInfos.empty()) {
        return DP_SUCCESS;
    ...
    for (const auto& subscriberInfo : subscriberInfos) {
        ...
        if (subscriberInfo.GetProfileChangeTypes().count(ProfileChangeType::ACCOUNT_ACL_ADD) != 0) {
            listenerProxy->OnAccountAclAdd(trustDeviceProfile);
        }
    }
    return DP_SUCCESS;
}

int32_t SubscribeProfileManager::NotifyAccountAclActive(const TrustDeviceProfile& trustDeviceProfile)
{
    auto subscriberInfos = GetSubscribeInfos(SUBSCRIBE_TRUST_DEVICE_PROFILE);
    if (subscriberInfos.empty()) {
        return DP_SUCCESS;
    ...
    for (const auto& subscriberInfo : subscriberInfos) {
        ...
        if (subscriberInfo.GetProfileChangeTypes().count(ProfileChangeType::ACCOUNT_ACL_ACTIVE) != 0) {
            listenerProxy->OnAccountAclActive(trustDeviceProfile);
        }
    }
    return DP_SUCCESS;
}
