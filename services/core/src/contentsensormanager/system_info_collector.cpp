...
int32_t SystemInfoCollector::GetOsType()
{
    ...
    if (strcmp(apiVersion, UNDEFINED_VALUE) != 0 || strcmp(bootSN, UNDEFINED_VALUE) != 0 ||
        ...
        HILOGI("apiVersion: %{public}s bootSN: %{public}s osFullName: %{public}s", ProfileUtils::GetAnonyString(apiVersion).c_str(),
            ProfileUtils::GetAnonyString(std::string(bootSN)).c_str(), ProfileUtils::GetAnonyString(osFullName).c_str());
        return OH_OS_TYPE;
    }
    ...
    if (strcmp(versionSDK, UNDEFINED_VALUE) != 0) {
        HILOGI("versionSDK: %{public}s", ProfileUtils::GetAnonyString(versionSDK).c_str());
        return HO_OS_TYPE;
    }
